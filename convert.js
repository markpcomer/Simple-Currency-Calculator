const currencies = {
  ...
  function fetchCurrencies() {
      ...
      fetch("https://v6.exchangerate-api.com/v6/e180883e3cd79ac9321b020c/latest/" + first)
        ...
        .then((data) => {
            console.log(data);
            displayCurrency(data, first, second, amount);
        })
        .catch((error) => console.error("FETCH ERROR:", error));

        function displayCurrency(data, first, second, amount) {
            const calculated = amount * data.conversion_rates[second];
            document.querySelector(".result").setAttribute("style", "display:block");
            document.querySelector(".result-currency1").innerHTML = amount + " " + first + "=";
            document.querySelector(".result-currency2").innerHTML = calculated + " " + second;
        }
